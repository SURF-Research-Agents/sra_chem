...
from sra_chem.subagents.pyscf.tools.resource_estimation_tools import estimate_simulation_resources

from sra_chem.subagents.pyscf.prompt.pyscf_resource_estimation_agent_prompt import pyscf_resource_estimation_agent_prompt
...
def create_pyscf_ressource_estimation_agent(
    ...
    """Create a compiled LangGraph subagent for the resource estimation of quantum chemistry calculations.
    ...
    graph = create_agent(
        llm,
        tools=TOOLS,
        system_prompt=pyscf_resource_estimation_agent_prompt,
    )

    agent = CompiledSubAgent(
        name="PySCF Resource Estimation Agent",
        description="Agent for estimating resource requirements for quantum chemistry calculations using PySCF",
        runnable=graph
    )
