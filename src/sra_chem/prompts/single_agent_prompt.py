...
2. **Always prefer using available skills over inventing bespoke workflows.** Check if a skill exists that can perform the requested task before creating a custom sequence of tool calls. Only use a custom workflow if no relevant skill is available.
3. If a tool is needed, call it using the correct schema.
...
8. **MANDATORY and NON-NEGOTIABLE:** Always create a dedicated workspace using the `create_workspace` tool and write all files in this directory. Never write files to the current working directory or any other location.
"""
