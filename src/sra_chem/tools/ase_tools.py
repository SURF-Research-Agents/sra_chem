...
from sra_chem.schemas.ase_input import (
    ASEInputSchema,
    ASEOutputSchema,
)
...
def load_calculator(calculator: dict) -> tuple[object, dict, dict]:
    ...
    if "emt" in calc_type:
        from sra_chem.schemas.calculators.emt_calc import EMTCalc

        calc = EMTCalc(**calculator)
    elif "tblite" in calc_type:
        from sra_chem.schemas.calculators.tblite_calc import TBLiteCalc

        calc = TBLiteCalc(**calculator)
    elif "orca" in calc_type:
        from sra_chem.schemas.calculators.orca_calc import OrcaCalc

        calc = OrcaCalc(**calculator)

    elif "nwchem" in calc_type:
        from sra_chem.schemas.calculators.nwchem_calc import NWChemCalc

        calc = NWChemCalc(**calculator)

    elif "fairchem" in calc_type:
        from sra_chem.schemas.calculators.fairchem_calc import FAIRChemCalc

        calc = FAIRChemCalc(**calculator)

    elif "mace" in calc_type:
        from sra_chem.schemas.calculators.mace_calc import MaceCalc

        calc = MaceCalc(**calculator)

    elif "aimnet2" in calc_type:
        from sra_chem.schemas.calculators.aimnet2_calc import AIMNET2Calc
